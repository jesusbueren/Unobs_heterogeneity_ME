subroutine value_fct_it(Ef_v,F,P,CCP,v_l,u_l,V_new)
    ...
    do while (dist>crit)
            ...
            !!!!!!!!!!!!!!
                !No attempt
                v_00(1:2*P-1)= beta*matmul(F(1:2*P-1,1:2*P-1,1,1),V_old(1:2*P-1,1))
                !Attempt               
                v_0I(1:2*P-1)=PI_s_v(1:2*P-1,1,P,v_l)*(-c_s+beta*(matmul(F(1:2*P-1,1:2*P-1,1,2),V_old(1:2*P-1,2)))) & !success
            ...
            !!Two wells (n=3)
            !!!!!!!!!!!!!!!!!!
                V_new(1:2*P-1,3)=Ef_v(1:2*P-1,3) &
