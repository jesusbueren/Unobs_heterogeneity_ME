subroutine compute_eq_F_CCP(params,F,CCP_mid,V_fct,V_social,n_initial,v_l,mean_N,social_output,private_output,Pr_u_X)
    ...
    double precision,dimension(3*P_max-2,3*P_max-2,4,4,P_max,unobs_types),intent(out)::F
    double precision,dimension(3*P_max-2,3,P_max,types_a,unobs_types),intent(inout)::CCP_mid
    integer,dimension(plots_in_map,1),intent(inout)::n_initial
    double precision,dimension(3*P_max-2,4,P_max,types_a,unobs_types),intent(inout)::V_fct,V_social
    integer,intent(in)::v_l    
    double precision,intent(out)::mean_N,social_output,private_output
    double precision,dimension(3*P_max-2,4,P_max,types_a,unobs_types),intent(out)::Pr_u_X
    double precision,dimension(3*P_max-2,3,P_max,types_a,unobs_types)::CCP_old,CCP,CCP2
    ...
    integer(8),dimension(3*P_max-2,4,4,P_max,unobs_types)::iterations
    character::pause_k
...
1    call generate_beliefs(CCP_mid,V_fct,V_social,Ef_v(:,:,:,:,v_l,:),n_initial,F,v_l,iterations,mean_N,social_output,private_output,Pr_u_X)
    ...
    do P_l=1,P_max; do u_l=1,unobs_types; do a_l=1,types_a
        call value_fct_it(Ef_v(1:3*P_l-2,:,P_l,a_l,v_l,u_l)&
                            ,F(1:3*P_l-2,1:3*P_l-2,:,:,P_l,u_l) &
                            ,P_l &
                            ,CCP(1:3*P_l-2,:,P_l,a_l,u_l),v_l,u_l &
                            ,V_fct(1:3*P_l-2,:,P_l,a_l,u_l))
        social=0
        call policy_fct_it(Ef_v(1:3*P_l-2,:,P_l,a_l,v_l,u_l)&
                            ,F(1:3*P_l-2,1:3*P_l-2,:,:,P_l,u_l) &
                            ,P_l &
                            ,CCP(1:3*P_l-2,:,P_l,a_l,u_l),CCP2(1:3*P_l-2,:,P_l,a_l,u_l),v_l,u_l &
                            ,V_fct(1:3*P_l-2,:,P_l,a_l,u_l),a_l)
        social=1
        call policy_fct_it(Ef_v(1:3*P_l-2,:,P_l,a_l,v_l,u_l)&
                            ,F(1:3*P_l-2,1:3*P_l-2,:,:,P_l,u_l) &
                            ,P_l &
                            ,CCP(1:3*P_l-2,:,P_l,a_l,u_l),CCP2(1:3*P_l-2,:,P_l,a_l,u_l),v_l,u_l &
                            ,V_social(1:3*P_l-2,:,P_l,a_l,u_l),a_l)
