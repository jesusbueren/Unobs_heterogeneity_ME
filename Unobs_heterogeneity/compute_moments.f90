subroutine compute_moments(data_in,string_name,moment_own_nxa)
    ...
    double precision,dimension(types_a,3),intent(out)::moment_own_nxa
    CHARACTER (LEN=4),intent(in) ::string_name
    double precision,dimension(max_NFW+1)::counter_N,moment_N
    double precision,dimension(types_a,3)::counter_own_nxa
    double precision,dimension(unobs_types,3)::counter_uhe,moment_uhe
    ...
    do i_l=1,plots_i
        if (impute_i(i_l)==0) then
            do t_l=1,T_sim
                if (data_in(t_l,i_l)/=-9.0d0 .and. n_data(t_l,i_l)<4 ) then
