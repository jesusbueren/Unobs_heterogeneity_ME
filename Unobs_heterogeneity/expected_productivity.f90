    ...
    do P=1,P_max  
        ...
        do m_l=1,M
            do p_l=1,2*P-1;do k_l=1,K
                ...
                Ef(p_l,m_l,2)=Ef(p_l,m_l,2)+(theta_p*(q(k_l,1)**beta_p*a_chosen**gamma_p)-cost_p*a_chosen)*PI_k(p_l,k_l,m_l,u_l) !Ef(1,1,2) Ef(2,1,3)
            end do;end do
            do p_l=2,2*P;do k_l=1,K;do k_l2=1,K
                Ef(p_l,m_l,3)=Ef(p_l,m_l,3)+theta_p*(q(k_l,1)**(beta_p/(1.0d0-gamma_p))+q(k_l2,1)**(beta_p/(1.0d0-gamma_p)))**(1.0d0-gamma_p)*a**gamma_p &
