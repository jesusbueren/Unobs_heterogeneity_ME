subroutine simulate_spatial_correlation()
    ...
    integer::v_l,i,j,ind,a_l,t_l,count_unique_mode,count_mode_equal_type
    double precision::u
    ...
    integer,dimension(plots_in_map,villages)::unobs_types_i_new
    ...
    do v_l=1,villages
        ...
        do i=1,plots_v(v_l)
            write(12,'(I3)'),unobs_types_i(i,v_l)
        end do
        close(12)
    ...
    do i=1,plots_v(v_l)
        ...
        do j=1,PA_type(i,1,v_l) 
            if (neighbors(i,j,v_l)/=i) then !neighbors(i,:,v_l)
                counter_type(unobs_types_i(neighbors(i,j,v_l),v_l))=counter_type(unobs_types_i(neighbors(i,j,v_l),v_l))+1
            end if
        end do
        ...
        elseif (counter_type(2)==counter_type(3) .and. counter_type(2)>counter_type(1)) then
            ...
            if (unobs_types_i(i,v_l)==maxloc(counter_type,1)) then
                count_mode_equal_type=count_mode_equal_type+1
            end if
        end if
        call RANDOM_NUMBER(u)
        if (u<pr_type(1)) then
            unobs_types_i_new(i,v_l)=1
        elseif (u<sum(pr_type(1:2))) then
            unobs_types_i_new(i,v_l)=2
        else
            unobs_types_i_new(i,v_l)=3
        end if
        counter_all(unobs_types_i_new(i,v_l))=counter_all(unobs_types_i_new(i,v_l))+1
        unobs_types_i(i,v_l)=unobs_types_i_new(i,v_l)
    end do
    !print '(I5,<4>F20.3)',t_l,dble(counter_all)/dble(plots_v(v_l)),dble(count_mode_equal_type)/dble(count_unique_mode)
    unobs_types_i(:,v_l)=unobs_types_i_new(:,v_l)
    end do
