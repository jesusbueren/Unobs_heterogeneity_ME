subroutine generate_beliefs(CCP,V_fct,V_social,Ef_v,n_initial,F_new,v_l,iterations,mean_N,social_output,private_output,Pr_u_x)
    ...
    implicit none
    double precision,dimension(3*P_max-2,3,P_max,types_a,unobs_types),intent(in)::CCP
    double precision,dimension(3*P_max-2,4,P_max,types_a,unobs_types),intent(in)::V_fct,V_social
    double precision,dimension(3*P_max-2,4,P_max,types_a,unobs_types),intent(in)::Ef_v 
    integer,dimension(plots_in_map,1),intent(inout)::n_initial
    double precision,dimension(3*P_max-2,3*P_max-2,4,4,P_max,unobs_types),intent(out)::F_new
    integer,intent(in)::v_l
    integer(8),dimension(3*P_max-2,4,4,P_max,unobs_types),intent(out)::iterations
    double precision,dimension(3*P_max-2,4,P_max,types_a,unobs_types),intent(out)::Pr_u_x !Pr_u_x(1,1,3,4,:) counter_u(1,1,3,4,:)
    integer(8),dimension(3*P_max-2,4,P_max,types_a,unobs_types)::counter_u
    integer(8),parameter::T=100000!0!150000
    integer(8),dimension(plots_in_map,3)::state,state_old
    integer(8)::i_l,j_l,t_l,ind,N_all,n_l,P,A,P_l,n_l2,it,m_l,it_min,a_l,u_l,it_new
    double precision::u_d,u_s,u_f,u_m,it2
    integer(8),parameter:: its=10000
    double precision,dimension(its)::NPV,total_N,NPV_PV,CCP_av
    double precision,intent(out)::mean_N,social_output,private_output
    integer(8),dimension(3*P_max-2,3*P_max-2,4,4,P_max,unobs_types)::beliefs_c
    integer(8),dimension(1)::seed=321,seed2
    integer(8),parameter::burn_t=10000
    double precision,dimension(3*P_max-2,3*P_max-2,4,4,P_max,unobs_types)::F
    double precision,dimension(P_max)::dist
    double precision,dimension(3*P_max-2)::CCP_aux
    character::continue_k
    
    
    
    CCP_aux=1.0d0/(1.0d0+exp(-(-PI_s_v(1:3*P_max-2,3,P_max,v_l)*c_s-(1.0d0-PI_s_v(1:3*P_max-2,3,P_max,v_l))*c_d)/rho(2)))
    !print*,'smthg'
    !Call seed number
    ...
    do t_l=1,T-1;   
        ...
        do i_l=1,plots_v(v_l)
            if (active_plots(i_l,v_l)==1) then
                ...
                do j_l=1,PA_type(i_l,1,v_l) !PA_type(i_l,1) stores the number of plots in the adjacency
                    ...
                    elseif (state(neighbors(i_l,j_l,v_l),1)==4)  then
                        N_all=N_all+3 !number of wells (there is three wells)
                    end if
                end do
                state(i_l,2)=N_all !second column in state: number of wells around
                ...
                elseif(n_l==3) then !two wells
                    ...
                    if (u_d<CCP(ind,n_l,P,A,unobs_types_i(i_l,v_l))) then !decides to drill
                        call RANDOM_NUMBER(u_s)
                         ...
                         do it_new=1,2    
                             call RANDOM_NUMBER(u_f)
                             if (u_f<PI_fm(N_all-1,m_l,unobs_types_i(i_l,v_l))) then
                                 n_initial(i_l,1)=n_initial(i_l,1)-1
                             end if
                         end do
                    else
                        call RANDOM_NUMBER(u_f)
            ...
            if (t_l>T-6) then
                if (n_l==1 .or. n_l==2 .or. n_l==3) then
        ...
        if (t_l>burn_t+1) then
            F=0.0d0
            do P_l=2,P_max; do ind=1,3*P_l-2; do n_l=1,4; do n_l2=1,4;do u_l=1,unobs_types
                    if ((n_l==1 .and. n_l2==3) .or. (n_l==1 .and. n_l2==4) .or. (n_l==2 .and. n_l2==4)) then
    ...
    do P_l=2,P_max; do ind=1,3*P_l-2; do n_l=1,4; do n_l2=1,min(n_l+1,4); do u_l=1,unobs_types
        it_min=300000
        if (iterations(ind,n_l,n_l2,P_l,u_l)==0) then
            F_new(ind,1:3*P_l-2,n_l,n_l2,P_l,u_l)=1.0d0/(2.0d0*dble(P_l)-1.0d0)
        end if
        if (isnan(F_new(ind,1,n_l,n_l2,P_l,u_l))) then
            print*,'error in generate beliefs'
        end if
        F_new(ind,1:3*P_l-2,n_l,n_l2,P_l,u_l)=F_new(ind,1:3*P_l-2,n_l,n_l2,P_l,u_l)/sum(F_new(ind,1:3*P_l-2,n_l,n_l2,P_l,u_l))
    ...
    P_l=1
    ind=1
    do n_l=1,4; do n_l2=1,min(n_l+1,4);do u_l=1,unobs_types
    ...
    do ind=1,3*P_max-2; do n_l=1,4; do P_l=1,P_max; do a_l=1,types_a; do u_l=1,unobs_types
