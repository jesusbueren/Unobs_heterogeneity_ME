subroutine valuation(CCP,C,Ef_v,P,V,v_l,u_l,a_l,CCP_aux)
    ...
    integer,intent(in)::P
    double precision,dimension((P)*2,(P)*2),intent(in)::C
    double precision,dimension(P,1),intent(in)::CCP
    double precision,dimension(P),intent(in)::CCP_aux
    double precision,dimension(P,2),intent(in)::Ef_v
    integer,intent(in)::v_l,u_l,a_l
    double precision,dimension((P)*2,1),intent(out)::V
    double precision,dimension((P)*2,1)::U
    double precision,dimension(P,2)::U_small
    integer::n_l,m_l,ind
