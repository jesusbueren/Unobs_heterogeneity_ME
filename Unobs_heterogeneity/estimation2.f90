...
subroutine estimation2(params_MLE,log_likeli)
    ...
    p_g(1,1:4)=(/25.36d0,0.33d0,0.14d0,8.0d0/)
    do p_l=2,par+1
        p_g(p_l,:)=p_g(1,:)
...
function log_likelihood2(params_MLE)
    ...
    double precision,dimension(2*P_max-1,2*P_max-1,3,P_max,villages,unobs_types)::joint_pr
        ...
        if (impute_i(i_l)==0) then
            ...
            if (P_type(i_l)>1) then !more than one neighbor
                ...
                do t_l=1,T_sim
                    ...
                    j_l=min(maxloc(Pr_N_data(:,t_l,i_l),1),2*(P_type(i_l)-1)+(n_data(t_l,i_l)-1)+1) !n_data(t_l,i_l),min(max_NFW+1,2*(P_type(i_l)-1)+n_data(t_l,i_l)) !Pr_N_data(:,t_l,4)
                        ...
                        if (drilling_it(t_l,i_l,s_l)==1 .and. n_data(t_l,i_l)<3) then 
                            ...
                            av_CCP_uhe(t_l,i_l,:)=av_CCP_uhe(t_l,i_l,:)
                        ...
                        if (t_l<T_sim) then
                            ...
                            elseif (n_data(t_l,i_l)==2.and. drilling_it(t_l,i_l,s_l)==0 .and. n_data(t_l+1,i_l)==2 ) then
                                ...
                                P_N2_N1=(1.0d0-PI_f_v(ind,n_data(t_l,i_l),P_type(i_l),V_type(i_l),:)) !PI_f_v(1:10,n_data(t_l,i_l),P_type(i_l),V_type(i_l),1)
                            ...
                            j_l2=min(maxloc(Pr_N_data(:,t_l+1,i_l),1),2*(P_type(i_l)-1)+(n_data(t_l+1,i_l)-1)+1)
                    ...
                    if (sum(joint_pr(ind,:,n_data(t_l,i_l),P_type(i_l),V_type(i_l),:))==0)then
                        print*,''
                    end if
                    
                    if (sum(likelihood_it*sum(joint_pr(ind,:,n_data(t_l,i_l),P_type(i_l),V_type(i_l),:),1) )==0.0d0) then
                        print*,''
                    end if

                    if (t_l==1) then
                        likelihood_it=likelihood_it*sum(joint_pr(ind,:,n_data(t_l,i_l),P_type(i_l),V_type(i_l),:),1) 
                    end if  
    ...
    !apanyo
    !if (missing_x1>10) then
    !    log_likelihood2=1.0d0/0.0d0
    !end if
