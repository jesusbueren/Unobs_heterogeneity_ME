subroutine policy_fct_it(Ef_v,F,P,CCP_in,CCP_out,v_l,u_l,V_new,a_l)
    ...
    double precision,dimension(3*P-2,3),intent(in)::CCP_in
    integer,intent(in)::v_l,u_l,a_l
    double precision,dimension(3*P-2,3),intent(out)::CCP_out
    double precision,dimension((3*P-2),4),intent(out)::V_new
    double precision,dimension(3*P-2)::CCP_aux
    double precision,dimension(3*P-2,3)::CCP_old
    double precision,dimension((3*P-2)*4,(3*P-2)*4)::C
    double precision,dimension((3*P-2)*4,1)::V
    ...
    CCP_aux=1.0d0/(1.0d0+exp(-(-PI_s_v(1:3*P-2,3,P,v_l)*c_s-(1.0d0-PI_s_v(1:3*P-2,3,P,v_l))*c_d)/rho(2)))
    ...
    dist=maxval(abs((reshape(CCP_old,(/(3*P-2)*3/))-reshape(CCP_out,(/(3*P-2)*3/)))))
    it=it+1
    ...
    if (it>1000) then
        print*,'P',P
        print*,CCP_old(1:3*P-2,1)
        print*,CCP_out(1:3*P-2,1)
