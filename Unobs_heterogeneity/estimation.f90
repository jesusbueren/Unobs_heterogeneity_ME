...
subroutine estimation(params_MLE,log_likeli)
    ...
    integer::iter,p_l,a_l,P_l2,v_l,ind,n_l,u_l,re_l
    interface 
    ...
    double precision,dimension(2*P_max-1,2,P_max,types_a,villages,unobs_types,re_types)::CCP_old,CCP_mid
    double precision,dimension(2*P_max-1,3,P_max,types_a,villages,unobs_types,re_types)::V_fct
    double precision,dimension(2*P_max-1,3,P_max,types_a,villages,unobs_types,re_types)::Ef_v !Ef_v: expected productivity
    double precision::dist,dz
    integer::it
...
1   n_initial_all(1:plots_in_map,:)=1    
    ...
    do re_l=1,re_types
        re_effect(re_l)=(-2.0d0+dz*dble(re_l-1))*params_MLE(4)
    end do    
    
    do v_l=1,villages
        do u_l=1,unobs_types;do a_l=1,types_a;do re_l=1,re_types
            call expected_productivity(params_MLE(1:3),area(a_l),Ef_v(:,:,:,a_l,v_l,u_l,re_l),v_l,u_l,re_l)
        end do;end do;end do
        do P_l=1,P_max; do a_l=1,types_a; do u_l=1,unobs_types;do re_l=1,re_types 
            call policy_fct_it(Ef_v(1:2*P_l-1,:,P_l,a_l,v_l,u_l,re_l)&
                                ,F_est(1:2*P_l-1,1:2*P_l-1,:,:,P_l,v_l) &
                                ,P_l &
                                ,CCP_old(1:2*P_l-1,:,P_l,a_l,v_l,u_l,re_l),CCP_est(1:2*P_l-1,:,P_l,a_l,v_l,u_l,re_l),v_l,u_l &
                                 ,V_fct(1:2*P_l-1,:,P_l,a_l,v_l,u_l,re_l))
    ...
    do P_l=1,P_max; do n_l=1,2;do ind=1,2*P_l-1; do v_l=1,villages
        dist=dist+dble(sum(iterations_all(ind,n_l,1:3,P_l,v_l)))/dble(sum(iterations_all(:,1:2,1:3,:,:)))*sum(abs(CCP_old(ind,n_l,P_l,:,v_l,:,:)-CCP_est(ind,n_l,P_l,:,v_l,:,:)))/dble(types_a)/dble(unobs_types)/dble(re_types)
    end do;end do; end do;end do
    print*,'dist',dist
...
function log_likelihood(params_MLE)
    ...
    double precision,dimension(2*P_max-1,2,P_max,types_a,villages,unobs_types,re_types)::CCP
    double precision,dimension(2*P_max-1,3,P_max,types_a,villages,unobs_types,re_types)::V_fct
    integer::i_l,t_l,type_l,a_l,p_l,v_l,ind,u_l,j_l,s_l,t,re_l
    double precision::log_likelihood,dz
    ...
    !$OMP PARALLEL default(shared) 
    !$OMP  DO
    do P_l=1,P_max; do a_l=1,types_a ; do u_l=1,unobs_types;do v_l=1,villages;do re_l=1,re_types
        call policy_fct_it(Ef_v(1:2*P_l-1,:,P_l,a_l,u_l,re_l)&
                        ,F_est(1:2*P_l-1,1:2*P_l-1,:,:,P_l,v_l) &
                        ,P_l &
                        ,CCP_est(1:2*P_l-1,:,P_l,a_l,v_l,u_l,re_l),CCP(1:2*P_l-1,:,P_l,a_l,v_l,u_l,re_l),v_l,u_l,V_fct(1:2*P_l-1,:,P_l,a_l,v_l,u_l,re_l))
        ...
        if (impute_i(i_l)==0) then
            ...
            if (P_type(i_l)>1) then !more than one neighbor
                ...
                if (UHE_type_model(1,1,i_l)==-9.0d0) then
                    UHE_type_model(:,:,i_l)=1.0d0/dble(unobs_types)/dble(re_types)
                end if
                log_likelihood=log_likelihood+log(sum(sum(likelihood_i*UHE_type_model(:,:,i_l),2),1))
